...
class Blog_Doc {
	...
	// Method to get files from their directory in Drive.
	async getFiles(dirName) {
		const files = (await drive.list({ prefix: dirName })).names
		return files
	}

	// Method to get the pages and posts data.
	async getMarkdownData(dirname) {
		// Get the files from their directory in Blog-Doc Drive
		...
		for (const file of mdFiles) {
			...
			const fileBlob = await drive.get(file)
	...
	async getPosts() {
		const postsData = await getMarkdownData("posts")
